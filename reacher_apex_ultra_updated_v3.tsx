...
function ReacherApp() {
  ...
  const playToneBurst = useCallback((freqs: number[], duration = 0.18) => {
    ...
    freqs.slice(0, 3).forEach((freq, index) => {
      ...
      const start = audioCtx.current!.currentTime + index * 0.07;
  ...
  const playSoftClick = useCallback((freq = 540) => {
    playToneBurst([freq, freq * 1.25], 0.12);
  }, [playToneBurst]);
      ...
      <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-center p-12 overflow-hidden text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 space-y-12">
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-center">
            <ApexButton size="lg" className="px-14 py-10 text-2xl gap-3" onClick={() => { playToneBurst([420, 560, 720], 0.18); setScreen("main"); }}><Dumbbell size={24} /> Workout Board</ApexButton>
            <ApexButton variant="outline" size="lg" className="px-14 py-10 text-2xl gap-3 border-teal-500/30 text-white" onClick={() => { playToneBurst([520, 660, 820], 0.18); setScreen("nutrition"); }}><Flame size={24} /> Nutrition</ApexButton>
          </div>
        </motion.div>
      </div>
    );
        ...
        <div className="max-w-7xl mx-auto space-y-10">
          <header className="flex justify-between items-center">
            ...
            <div className="flex gap-3">
              <ApexButton variant="outline" className="gap-2" onClick={() => { playSoftClick(460); setScreen("splash"); }}><Home size={18} /> מסך פתיחה</ApexButton>
              <ApexButton variant="premium" className="gap-2" onClick={() => { playToneBurst([700, 880], 0.14); setScreen("main"); }}><ArrowRightLeft size={18} /> לעבור לאפליקציה</ApexButton>
            </div>
          </header>
    ...
    <div className="min-h-screen bg-[#020617] text-slate-100 font-sans selection:bg-teal-500/30 overflow-x-hidden" dir="rtl">
      ...
      <header className="relative z-10 max-w-7xl mx-auto px-6 pt-12 pb-8 flex justify-between items-start">
        ...
        <div className="flex gap-4">
          ...
          <ApexButton variant="outline" size="icon" className="rounded-2xl border-white/5" onClick={() => { playToneBurst([510, 640, 780], 0.12); window.open("spotify://", "_blank"); }}><Music size={22} /></ApexButton>
        </div>
      </header>
