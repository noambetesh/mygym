...
function ReacherApp() {
  ...
  const playToneBurst = useCallback((freqs: number[], duration = 0.16) => {
    ...
    freqs.slice(0, 2).forEach((freq, index) => {
      ...
      const start = audioCtx.current!.currentTime + index * 0.09;
  ...
  const playAlert = (f: number) => {
    ...
    g.gain.exponentialRampToValueAtTime(0.03, audioCtx.current.currentTime + 0.02);
    g.gain.exponentialRampToValueAtTime(0.0001, audioCtx.current.currentTime + 0.18);
    o.start();
    o.stop(audioCtx.current.currentTime + 0.18);
  };
      ...
      <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-center p-12 overflow-hidden text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 space-y-12">
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-center">
            <ApexButton size="lg" className="px-20 py-10 text-2xl" onClick={() => { playToneBurst([440, 620, 880], 0.16); setScreen("nutrition"); }}>START</ApexButton>
          </div>
        </motion.div>
      </div>
    );
