...
function ReacherApp() {
  ...
  const playToneBurst = useCallback((freqs: number[], duration = 0.18) => {
    if (typeof window === "undefined") return;
    ...
    freqs.forEach((freq, index) => {
      ...
      const start = audioCtx.current!.currentTime + index * 0.06;
  ...
  useEffect(() => {
    ...
    playToneBurst(screen === "nutrition" ? [520, 660, 780] : [440, 620, 820], 0.14);
      ...
      <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-center p-12 overflow-hidden text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 space-y-12">
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-center">
            <ApexButton size="lg" className="px-20 py-10 text-2xl" onClick={() => { playToneBurst([440, 620, 880], 0.16); setScreen("nutrition"); }}>ENTER HUB</ApexButton>
          </div>
        </motion.div>
      </div>
    );
      ...
      <div className="min-h-screen bg-[#020617] text-white p-6 md:p-12" dir="rtl">
        <div className="max-w-7xl mx-auto space-y-10">
          <header className="flex justify-between items-center">
            ...
            <ApexButton variant="premium" className="gap-2" onClick={() => { playToneBurst([700, 880], 0.14); setScreen("main"); }}><ArrowRightLeft size={18} /> לעבור לאימון</ApexButton>
          </header>
      ...
      <header className="relative z-10 max-w-7xl mx-auto px-6 pt-12 pb-8 flex justify-between items-start">
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 text-teal-500 font-black text-[10px] tracking-widest mt-2 uppercase">
            <div className="w-1.5 h-1.5 bg-teal-500 rounded-full animate-ping" /> System Online: Betesh Training
          </div>
        </div>
        <div className="flex gap-4">
          <ApexButton variant="outline" className="gap-2 rounded-2xl border-white/5 px-5" onClick={() => { playToneBurst([520, 660, 780], 0.14); setScreen("nutrition"); }}><HeartPulse size={20} /> תזונה</ApexButton>
          <ApexButton variant="outline" size="icon" className="rounded-2xl border-white/5" onClick={() => { playToneBurst([740, 980], 0.12); window.open("spotify://", "_blank"); }}><Music size={22} /></ApexButton>
                ...
                {dayRecommendations.map(ex => (
                  <ApexCard key={ex.id} className="p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <ApexButton variant="outline" className="gap-2" onClick={() => { playToneBurst([520, 660, 780], 0.14); setScreen("nutrition"); }}><HeartPulse size={16} /> תזונה</ApexButton>
                    </div>
                  </ApexCard>
                ))}
              </div>
