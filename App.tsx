...
const NUTRITION_ARTICLES = [
  { title: "Protein and Exercise - Better Health", url: "https://www.nhs.uk/live-well/eat-well/food-guidelines-and-food-labels/the-eatwell-guide/" , tag: "בסיס תזונה" },
  { title: "Hydration for Exercise", url: "https://www.cdc.gov/healthy-weight-growth/water-healthy-drinks/index.html", tag: "מים" },
  { title: "Physical Activity Guidelines", url: "https://health.gov/moveyourway/get-the-facts/adults", tag: "אימון" },
  { title: "Healthy Eating Plate", url: "https://nutritionsource.hsph.harvard.edu/healthy-eating-plate/", tag: "ארוחות" },
];
...
function generateWorkoutChatReply({
  question,
  exercise,
  sessionList,
  logs,
  ...
  const focusExercise = exercise || sessionList[0];
  if (!focusExercise) {
    return "כדי לייעץ מדויק, כדאי לבחור תרגיל או לבנות קודם אימון קצר עם 3-5 תרגילים.";
  }

  const latestLog = getExerciseLogs(focusExercise.id, logs)[0];
  const lower = question.toLowerCase();

  const targetAdvice =
    lower.includes("next") || lower.includes("הבא") || lower.includes("מה עכשיו")
      ? `התרגיל הבא שכדאי לך להתמקד בו הוא ${focusExercise.name}. תתחיל בסט ראשון נקי ורק אחרי זה תעלה קצב.`
      : lower.includes("pain") || lower.includes("כאב")
        ? "אם יש כאב חד, עצור את התרגיל, בדוק טווח תנועה וטכניקה, ואל תנסה לדחוף עומס בכוח."
        : lower.includes("weight") || lower.includes("משקל")
          ? getOverloadSuggestion(focusExercise, logs)
          : "הכי נכון עכשיו הוא שליטה, טווח מלא, וניהול עומס חכם.";

  const historyAdvice = latestLog
    ? `לפי התיעוד האחרון, עשית ${latestLog.weight} ק״ג ל-${latestLog.reps} חזרות.`
    : "עדיין אין תיעוד על התרגיל הזה, אז תתחיל במשקל שמאפשר טכניקה יציבה.";

  return `Workout Coach: ${focusExercise.name}. ${historyAdvice} ${targetAdvice} בנוסף, תחשוב על איכות תנועה לפני מהירות.`;
}

function getDateKeyFromHistoryItem(item: SessionData) {
...
function buildCalendarMaps(
  ...
  const mealMap = new Map<string, NutritionEntry[]>();
  nutritionEntries.forEach((entry) => {
    const key = formatDayKey(new Date(entry.createdAt));
    mealMap.set(key, [...(mealMap.get(key) || []), entry]);
  });

  const workoutMap = new Map<string, SessionData[]>();
  history.forEach((item) => {
    const key = getDateKeyFromHistoryItem(item);
    workoutMap.set(key, [...(workoutMap.get(key) || []), item]);
  });
  ...
  bodyMeasurements.forEach((item) => {
    ...
    measurementMap.set(key, [...(measurementMap.get(key) || []), item]);
  });

  const photoMap = new Map<string, BodyPhoto[]>();
  bodyPhotos.forEach((item) => {
    const key = formatDayKey(new Date(item.createdAt));
    photoMap.set(key, [...(photoMap.get(key) || []), item]);
  });

  return { mealMap, workoutMap, measurementMap, photoMap };
}
...
function NutritionCalendarCard({
  ...
  mealMap: Map<string, NutritionEntry[]>;
  workoutMap: Map<string, SessionData[]>;
  measurementMap: Map<string, BodyMeasurement[]>;
  photoMap: Map<string, BodyPhoto[]>;
}) {
  ...
  const selectedWorkouts = workoutMap.get(selectedDateKey) || [];
  const selectedMeasurements = measurementMap.get(selectedDateKey) || [];
  const selectedPhotos = photoMap.get(selectedDateKey) || [];
    ...
    <Card className="p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <div className="text-sm text-slate-400 mt-1">יומן מרכזי של אימונים, ארוחות, מדידות ותמונות</div>
        </div>
      ...
      <div className="grid grid-cols-7 gap-2">
        {cells.map((date) => {
          ...
          const hasMeal = mealMap.has(key);
          const hasWorkout = workoutMap.has(key);
          const hasMeasurement = measurementMap.has(key);
          const hasPhoto = photoMap.has(key);
          ...
          return (
            <button
              ...
              key={key}
              className={`min-h-[88px] rounded-2xl border p-2 flex flex-col justify-between text-right transition-all ${
                inMonth ? "bg-black/25 border-white/5" : "bg-black/10 border-white/0 opacity-45"
              } ${isToday ? "ring-1 ring-teal-400/50" : ""} ${isSelected ? "border-teal-400/60 bg-teal-500/10" : ""}`}
            >
              ...
              <div className="flex flex-wrap gap-1 justify-end">
                ...
                {hasMeasurement && <span className="text-[10px] px-2 py-1 rounded-full bg-amber-500/15 text-amber-300">מדידה</span>}
                {hasPhoto && <span className="text-[10px] px-2 py-1 rounded-full bg-pink-500/15 text-pink-300">תמונה</span>}
              </div>
            </button>
          );
        })}
      ...
      <div className="mt-6 grid lg:grid-cols-2 gap-6">
        <Card className="p-5 bg-black/30 border-white/5">
          <div className="text-sm text-slate-400 mb-3">פירוט ליום שנבחר</div>
          <div className="text-lg font-black italic mb-4">{selectedDateKey}</div>
          <div className="space-y-3">
            <div>
              <div className="text-xs text-teal-300 mb-2">ארוחות</div>
              {selectedMeals.length ? selectedMeals.map((meal) => (
                <div key={meal.id} className="bg-white/5 rounded-2xl px-4 py-3 text-sm mb-2">
                  {meal.mealLabel} - {meal.title} · {meal.calories} קל'
                </div>
              )) : <div className="text-sm text-slate-500">אין ארוחות מתועדות</div>}
            </div>
            <div>
              <div className="text-xs text-violet-300 mb-2">אימונים</div>
              {selectedWorkouts.length ? selectedWorkouts.map((workout) => (
                <div key={workout.id} className="bg-white/5 rounded-2xl px-4 py-3 text-sm mb-2">
                  {workout.exercises} תרגילים · נפח {workout.volume}
                </div>
              )) : <div className="text-sm text-slate-500">אין אימון מתועד</div>}
            </div>
          </div>
        </Card>

        <Card className="p-5 bg-black/30 border-white/5">
          <div className="text-sm text-slate-400 mb-3">עוד תיעוד</div>
          <div className="space-y-3">
            <div>
              <div className="text-xs text-amber-300 mb-2">מדידות</div>
              {selectedMeasurements.length ? selectedMeasurements.map((m) => (
                <div key={m.id} className="bg-white/5 rounded-2xl px-4 py-3 text-sm mb-2">
                  {m.weight ? `משקל ${m.weight}` : "מדידה"} {m.waist ? `· מותן ${m.waist}` : ""} {m.arm ? `· יד ${m.arm}` : ""}
                </div>
              )) : <div className="text-sm text-slate-500">אין מדידות באותו יום</div>}
            </div>
            <div>
              <div className="text-xs text-pink-300 mb-2">תמונות גוף</div>
              {selectedPhotos.length ? (
                <div className="grid grid-cols-3 gap-2">
                  {selectedPhotos.map((photo) => (
                    <SafeImage key={photo.id} src={photo.imageUrl} alt={photo.note || "photo"} className="w-full h-20 rounded-xl object-cover" />
                  ))}
                </div>
              ) : <div className="text-sm text-slate-500">אין תמונות באותו יום</div>}
            </div>
          </div>
        </Card>
      </div>
...
function CoachChatModal({
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[845] bg-black/85 backdrop-blur-md flex items-center justify-center p-4" dir="rtl">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-2xl bg-slate-900 border border-white/10 rounded-[2.3rem] overflow-hidden shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-white/5">
          <div className="flex items-center gap-3">
            <BrainCircuit className="text-teal-400" />
            <div className="text-2xl font-black italic">{title}</div>
          </div>
          <button onClick={onClose} className="w-10 h-10 rounded-full bg-white/10 text-white flex items-center justify-center"><X size={18} /></button>
        </div>
        <div className="p-5 max-h-[60vh] overflow-y-auto space-y-3">
          {messages.map((message) => (
            <div key={message.id} className={`flex ${message.role === "user" ? "justify-start" : "justify-end"}`}>
              <div className={`max-w-[80%] rounded-[1.4rem] px-4 py-3 text-sm leading-7 ${message.role === "user" ? "bg-white/8 text-white border border-white/10" : "bg-teal-500/15 text-teal-100 border border-teal-400/20"}`}>
                {message.text}
              </div>
            </div>
          ))}
        ...
        <div className="p-5 border-t border-white/5 flex gap-3">
          <button onClick={onSend} className="h-12 w-12 rounded-2xl bg-teal-500 text-slate-950 flex items-center justify-center shrink-0">
            <Send size={18} />
          </button>
          <input
            value={input}
            ...
            className="flex-1 h-12 bg-black/30 border border-white/10 rounded-2xl px-4 outline-none"
          />
        </div>
      </motion.div>
    </div>
...
function ConfettiBurst({ show }: { show: boolean }) {
  ...
  return (
    <AnimatePresence>
      {show && (
        <div className="fixed inset-0 pointer-events-none z-[860] overflow-hidden">
          {pieces.map((piece) => {
            const left = (piece * 97) % 100;
            const delay = (piece % 9) * 0.06;
            const duration = 1.8 + (piece % 5) * 0.22;
            const rotate = 90 + piece * 17;
            return (
              <motion.span
                key={piece}
                initial={{ y: -30, x: 0, opacity: 1, rotate: 0 }}
                animate={{ y: window.innerHeight + 80, x: (piece % 2 === 0 ? 120 : -120), opacity: 0.9, rotate }}
                exit={{ opacity: 0 }}
                transition={{ duration, delay }}
                className="absolute top-0 w-3 h-6 rounded-full"
                style={{
                  left: `${left}%`,
                  background: piece % 4 === 0 ? "#2dd4bf" : piece % 4 === 1 ? "#fb7185" : piece % 4 === 2 ? "#f59e0b" : "#818cf8",
                }}
              />
            );
          })}
...
function VictoryOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[855] bg-black/75 backdrop-blur-sm flex items-center justify-center p-4" dir="rtl">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-lg rounded-[2.2rem] bg-slate-900 border border-white/10 p-8 text-center shadow-2xl">
        <PartyPopper className="mx-auto text-teal-400 mb-4" size={40} />
        <div className="text-3xl font-black italic mb-3">אימון הושלם כמו מלך</div>
        <div className="text-slate-300 leading-7 mb-6">שמירה, תיעוד, וסיום נקי. עכשיו הזמן להתאוששות טובה, מים, וחלבון.</div>
        <div className="flex justify-center gap-3">
          <Btn variant="spotify" onClick={() => window.open("https://open.spotify.com/playlist/37i9dQZF1DX76Wlfdnj7AP", "_blank", "noopener,noreferrer")}>Victory Playlist</Btn>
          <Btn variant="premium" onClick={onClose}>סגור</Btn>
        </div>
      </motion.div>
    </div>
  );
...
function ReacherApp() {
  ...
  const [selectedCalendarDateKey, setSelectedCalendarDateKey] = useState(() => formatDayKey(new Date()));
  const [showNutritionChat, setShowNutritionChat] = useState(false);
  const [showWorkoutChat, setShowWorkoutChat] = useState(false);
  const [nutritionChatInput, setNutritionChatInput] = useState("");
  const [workoutChatInput, setWorkoutChatInput] = useState("");
  const [nutritionChatMessages, setNutritionChatMessages] = useState<CoachMessage[]>([{ id: "nutrition-welcome", role: "assistant", text: "אני כאן לייעוץ תזונה חכם. אפשר לשאול על ארוחות, חלבון, מים, יעד קלורי, או איך לבנות יום טוב יותר." }]);
  const [workoutChatMessages, setWorkoutChatMessages] = useState<CoachMessage[]>([{ id: "workout-welcome", role: "assistant", text: "אני כאן לייעוץ אימון אמיתי. אפשר לשאול על התרגיל הבא, עומס, טכניקה, סידור האימון, או מה עדיף עכשיו." }]);
...
const profiles = {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setShowVictoryOverlay(false), 5200);
    return () => window.clearTimeout(t);
  }, [showVictoryOverlay]);
...
const workoutProgressText = useMemo(() => getWorkoutProgressText(sessionList, logs), [sessionList, logs]);
  ...
  const handlePhaseTransition = () => {
    ...
    setHistory((prev) => [{ id: Math.random().toString(36).slice(2, 9), date: new Date().toLocaleDateString("he-IL"), dateKey: formatDayKey(new Date()), volume: totalVolume, exercises: sessionList.length }, ...prev]);
  ...
  const sendNutritionChat = () => {
    if (!nutritionChatInput.trim()) return;
    const userText = nutritionChatInput.trim();
    const reply = `${generateNutritionCoachReply({
      profile: nutritionProfile,
      calories: totalNutritionCalories,
      targetCalories: recommendedCalories,
      protein: totalProtein,
    ...
    setNutritionChatMessages((prev) => [...prev, { id: `user-${Date.now()}`, role: "user", text: userText }, { id: `assistant-${Date.now()}`, role: "assistant", text: reply }]);
    setNutritionChatInput("");
  };

  const sendWorkoutChat = () => {
    if (!workoutChatInput.trim()) return;
    const userText = workoutChatInput.trim();
    const reply = generateWorkoutChatReply({
      question: userText,
      exercise: currentExercise || exercisePreview || askAIExercise,
      sessionList,
      logs,
    });
    setWorkoutChatMessages((prev) => [...prev, { id: `wu-${Date.now()}`, role: "user", text: userText }, { id: `wa-${Date.now()}`, role: "assistant", text: reply }]);
    setWorkoutChatInput("");
  };
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2"><Btn variant="outline" className="h-10 px-3" onClick={() => setShowSessionQueue(true)}>רשימת אימון</Btn><Btn variant="outline" className="h-10 px-3" onClick={() => setShowWorkoutChat(true)}>צ׳אט מאמן</Btn><Dumbbell className="text-teal-400" /></div>
            ...
            {!inSession ? (
              ...
              <div className="space-y-4">
                <Card className="p-4 bg-white/5 border-white/10"><div className="flex items-center justify-between gap-4 flex-wrap"><ProgressRing value={completedExercises} total={Math.max(sessionList.length,1)} label="אימון" sublabel={workoutProgressText} size={88} stroke={9} /><div className="flex-1 min-w-[180px]"><div className="text-sm text-slate-300 mb-2">השריר הבא בפוקוס</div><div className="text-lg font-black italic text-teal-300">{currentPlanExercises[0]?.name || sessionList[0]?.name || "בחר תרגיל"}</div><div className="text-xs text-slate-400 mt-1">{remainingByMuscle.length ? `נשארו ${remainingByMuscle.reduce((acc,item)=>acc+item.left,0)} תרגילים` : "עדיין אין שרירים באימון"}</div></div></div></Card>
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Btn variant="outline" onClick={() => setShowWorkoutChat(true)}>צ׳אט מאמן</Btn>
  ...
  <div className="flex items-center justify-between mb-4">
    ...
    <Btn variant="outline" className="h-10" onClick={() => setShowSessionQueue(true)}>רשימת אימון</Btn>
    <Btn
  ...
  />
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        ...
        {filteredVault.map((ex) => (
          ...
          <Card key={ex.id} className="overflow-hidden cursor-pointer" onClick={() => setExercisePreview(ex)}>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Btn variant="premium" onClick={(e) => { e.stopPropagation(); addExerciseToSession(ex); }}><Plus size={15} /> אוסף תרגיל לאימון</Btn>

                <Btn variant="outline" onClick={() => setExercisePreview(ex)}><Dumbbell size={15} /> כרטיס תרגיל</Btn>

                <Btn variant="youtube" onClick={(e) => { e.stopPropagation(); window.open(ex.videoUrl, "_blank", "noopener,noreferrer"); }}><Youtube size={15} /> הדרכה</Btn>

                <Btn variant="outline" onClick={(e) => { e.stopPropagation(); addExerciseToCustomDay(selectedDay, ex.id); }}><CalendarDays size={15} /> הוסף לתכנון שבועי</Btn>
  ...
  const renderNutrition = () => (
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
      ...
      <Card className="p-6">
        <div className="flex items-center gap-3 mb-4"><BookOpen className="text-amber-400" /><h3 className="text-2xl font-black italic">מאמרים מעניינים</h3></div>
        <div className="space-y-3">{NUTRITION_ARTICLES.map((article) => <button key={article.url} onClick={() => window.open(article.url, "_blank", "noopener,noreferrer")} className="w-full text-right bg-black/30 border border-white/5 rounded-2xl px-4 py-4 hover:border-teal-400/40 transition-all"><div className="text-xs text-teal-300 mb-1">{article.tag}</div><div className="font-black italic text-white">{article.title}</div><div className="text-xs text-slate-400 mt-2">פתח מקור</div></button>)}</div>
      </Card>
      </div>
 ...
 return (
  ...
  <>
    ...
    <CoachChatModal
      open={showNutritionChat}
      title="Nutrition Chat"
      placeholder="שאל על קלוריות, חלבון, ארוחה הבאה או מים"
      messages={nutritionChatMessages}
      input={nutritionChatInput}
      setInput={setNutritionChatInput}
      onSend={sendNutritionChat}
      onClose={() => setShowNutritionChat(false)}
    />

    <CoachChatModal
      open={showWorkoutChat}
      title="Workout Chat"
      placeholder="שאל על התרגיל הבא, עומס, סדר אימון או טכניקה"
      messages={workoutChatMessages}
      input={workoutChatInput}
      setInput={setWorkoutChatInput}
      onSend={sendWorkoutChat}
      onClose={() => setShowWorkoutChat(false)}
    />

    <ConfettiBurst show={showVictoryOverlay} />
    <VictoryOverlay open={showVictoryOverlay} onClose={() => setShowVictoryOverlay(false)} />
