...
/**
 * BETESH TRAINING v21.5 - ULTRA PERFORMANCE SUITE
 * ENGINEERED FOR NOAM.
...
const NUTRITION_HERO = "https://images.unsplash.com/photo-1490645935967-10de6ba17061?q=80&w=1600";
...
const getYoutubeSearchUrl = (exerciseName: string) => `https://www.youtube.com/results?search_query=${encodeURIComponent(exerciseName + " exercise tutorial")}`;

function getExerciseVideoUrl(exercise: Exercise) {
  return getYoutubeSearchUrl(exercise.name);
}

function getFallbackImage(group: MuscleGroup) {
  return muscleGroupImages[group] || REACHER_HERO;
}

function getExerciseImageUrl(exercise: Exercise) {
  return exercise.imageUrl && exercise.imageUrl.trim() ? exercise.imageUrl : getFallbackImage(exercise.muscleGroup);
}

function playUiToneSequence(ctx: AudioContext, sequence: number[], stepMs = 110, gainValue = 0.05) {
...
function ensureAudioContext(audioRef: React.MutableRefObject<AudioContext | null>) {
  const Ctor = getBrowserAudioContext();
  if (!Ctor) return null;
  try {
    if (!audioRef.current) {
...
type MainTab = "dashboard" | "vault" | "stats" | "settings";
...
const APP_IDEAS = [
  ...
  "AI Notes לכל תרגיל בנפרד",
  "מסך תזונה מלא עם hydration, supplements וארוחות לדוגמה",
  "מעבר מהיר בין Hub תזונה ללוח האימונים",
  "צלילי מערכת שונים לפעולות שונות"
];
    ...
    <div className="fixed inset-0 z-[700] bg-black/90 backdrop-blur-2xl flex items-center justify-center p-6" dir="rtl">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-3xl bg-slate-900 border border-white/10 rounded-[3rem] overflow-hidden">
        <div className="relative h-56">
          <img src={getExerciseImageUrl(exercise)} className="w-full h-full object-cover opacity-30" onError={(e) => { e.currentTarget.src = getFallbackImage(exercise.muscleGroup); }} />
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <ApexButton variant="outline" className="flex-1 gap-2" onClick={() => window.open(getExerciseVideoUrl(exercise), "_blank")}><Youtube size={18} /> פתח וידאו</ApexButton>
          </div>
        </div>
      </motion.div>
    </div>
          ...
          <div className="grid md:grid-cols-2 gap-4 max-h-[60vh] overflow-y-auto pr-2 scrollbar-hide">
            {alts.map(ex => (
              <ApexCard key={ex.id} className="p-4 flex items-center gap-4 cursor-pointer hover:bg-white/5" onClick={() => { onSwap(ex); onClose(); }}>
                <img src={getExerciseImageUrl(ex)} className="w-20 h-20 rounded-2xl object-cover opacity-50" onError={(e) => { e.currentTarget.src = getFallbackImage(ex.muscleGroup); }} />
...
function ReacherAppInner() {
  ...
  useEffect(() => {
    if (screen === "nutrition") playSound("enter");
    if (screen === "main") playSound("enter");
  }, [screen]);

  useEffect(() => {
    if (tab === "vault" || tab === "stats") playSound("swap");
  }, [tab]);
  ...
  const playAlert = (f: number) => {
    const ctx = ensureAudioContext(audioCtx);
    if (!ctx) return;
    playUiToneSequence(ctx, [f], 120, 0.08);
  };

  const playSound = (kind: "enter" | "add" | "start" | "save" | "finish" | "swap") => {
    const ctx = ensureAudioContext(audioCtx);
    if (!ctx) return;
    const bank = {
      enter: [520, 660, 840],
      ...
      start: [480, 620, 820],
      save: [600, 780],
      finish: [540, 700, 880, 1040],
      swap: [500, 430, 620]
    } as const;
    playUiToneSequence(ctx, bank[kind], 100, kind === "finish" ? 0.06 : 0.05);
  };
      ...
      <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-center p-12 overflow-hidden text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 space-y-12">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-teal-400 font-mono text-[10px] uppercase tracking-[0.6em] mt-4"><Cpu size={14} className="inline mr-2" /> BETESH TRAINING ENGINE v21.5</p>
          </div>
          <h1 className="text-8xl md:text-[11rem] font-black italic uppercase text-white tracking-tighter leading-[0.8]">BETESH<br /><span className="text-teal-500">TRAINING</span></h1>
          <p className="text-slate-500 font-bold uppercase tracking-[0.4em] text-sm">Created by Noam Betesh - training, nutrition, recovery, progression.</p>
          <div className="flex flex-col md:flex-row gap-4 justify-center">
            <ApexButton size="lg" className="px-16 py-10 text-2xl" onClick={() => setScreen("nutrition")}>Nutrition Hub</ApexButton>
            <ApexButton variant="outline" size="lg" className="px-16 py-10 text-2xl" onClick={() => setScreen("main")}>Training Board</ApexButton>
          </div>
        </motion.div>
      </div>
    );
  ...
  if (screen === "nutrition") {
    return (
      <div className="min-h-screen bg-[#020617] text-white p-6 md:p-12 relative overflow-hidden" dir="rtl">
        <div className="absolute inset-0 opacity-10"><img src={NUTRITION_HERO} className="w-full h-full object-cover" /></div>
        <div className="max-w-7xl mx-auto space-y-10">
          <header className="flex justify-between items-center">
            <div>
              <p className="text-teal-500 font-black text-[10px] uppercase tracking-[0.5em] mb-2">Fuel system</p>
              <h2 className="text-6xl font-black italic uppercase tracking-tighter">Nutrition<br />Command</h2>
            </div>
            <ApexButton variant="premium" className="gap-2" onClick={() => setScreen("main")}><ArrowRightLeft size={18} /> לעבור לאימון</ApexButton>
          </header>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <ApexCard className="p-8 space-y-5">
              <h3 className="text-2xl font-black italic">Hydration Planner</h3>
              <div className="space-y-3 text-slate-300">
                <div>- בוקר: 500-700 מ"ל מים</div>
                <div>- לפני אימון: 400-600 מ"ל</div>
                <div>- תוך כדי אימון: לגימות קבועות</div>
                <div>- אחרי אימון: השלמת נוזלים + מעט מלח אם הזעת הרבה</div>
              </div>
            </ApexCard>
            <ApexCard className="p-8 space-y-5">
              <h3 className="text-2xl font-black italic">Meal Builder</h3>
              <div className="space-y-3 text-slate-300">
                <div>- פרה וורקאאוט: יוגורט, בננה, גרנולה</div>
                <div>- פוסט וורקאאוט: אורז, עוף, ירקות</div>
                <div>- ערב התאוששות: ביצים, טחינה, סלט, לחם מלא</div>
                <div>- נשנוש חכם: קוטג', פרי, שקדים</div>
              </div>
            </ApexCard>
            <ApexCard className="p-8 space-y-5">
              <h3 className="text-2xl font-black italic">Supplement Notes</h3>
              <div className="space-y-3 text-slate-300">
                <div>- קריאטין: 3-5 גרם ביום בעקביות</div>
                <div>- קפאין: לפני אימון רק אם זה מתאים לך</div>
                <div>- אבקת חלבון: כלי נוחות, לא חובה</div>
                <div>- אומגה 3 ומגנזיום: יכולים לעזור בשגרה</div>
              </div>
            </ApexCard>
          </div>
          <ApexCard className="p-8 bg-teal-500/5 border-teal-500/20">
            <div className="flex flex-col md:flex-row justify-between gap-6 items-center">
              <div>
                <p className="text-teal-400 font-black text-[10px] uppercase tracking-widest mb-2">Bridge mode - Betesh Training</p>
                <h3 className="text-3xl font-black italic">Betesh Training - תזונה ואימון באותו מקום</h3>
                <p className="text-slate-300 mt-2">מהמסך הזה אתה מקבל עוד שכבה, לא במקום האימון, אלא לפניו. זה ההאב שלך.</p>
              </div>
              <ApexButton variant="premium" size="lg" onClick={() => setScreen("main")}>לעבור ללוח האימונים</ApexButton>
            </div>
          </ApexCard>
        </div>
      </div>
            ...
            <ApexCard className="p-10 w-full max-w-sm space-y-8 bg-slate-900/40 border-white/5">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <ApexButton variant="outline" className="w-full h-16 text-sm italic" onClick={() => setAskAIExercise(ex)}>askAI</ApexButton>
                </div>
              </div>
            </div>
          </ApexCard>
      ...
      <header className="relative z-10 max-w-7xl mx-auto px-6 pt-12 pb-8 flex justify-between items-start">
        <div className="space-y-3">
          <div className="flex items-center gap-4">
            ...
            <ApexButton variant="premium" className="gap-2 rounded-[1.4rem]" onClick={() => setAskAIExercise(dayRecommendations[0] || MASTER_VAULT[0])}><span className="font-black tracking-widest">askAI</span></ApexButton>
          </div>
          <div className="flex items-center gap-2 text-teal-500 font-black text-[10px] tracking-widest mt-2 uppercase">
            <div className="w-1.5 h-1.5 bg-teal-500 rounded-full animate-ping" /> Created by Noam Betesh
          </div>
        </div>
        <div className="flex gap-4">
          <ApexButton variant="outline" size="icon" className="rounded-2xl border-white/5" onClick={() => setScreen("nutrition")}><HeartPulse size={22} /></ApexButton>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pb-40">
        <div className="flex bg-slate-900/80 p-2 rounded-[2.5rem] w-full max-w-2xl mx-auto mb-16 border border-white/10 backdrop-blur-3xl shadow-2xl">
          {[
            ...
            <button
              ...
              style={{ fontFamily: "Inter, Poppins, ui-sans-serif, system-ui, sans-serif", letterSpacing: "0.02em" }}
            >
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
            ...
            <div className="grid lg:grid-cols-[1fr_280px] gap-6 items-start">
                ...
                <div className="flex overflow-x-auto gap-3 pb-3">
                  {DAY_SPLITS.map(day => (
                    <button
                      ...
                      className={`px-6 py-4 rounded-[1.4rem] whitespace-nowrap font-black transition-all ${selectedDay === day.id ? "bg-teal-500 text-slate-950 shadow-2xl" : "bg-slate-900 text-slate-400 border border-white/5"}`}
                    >
                      {day.label}
                    </button>
                  ))}
                  ...
                  {dayRecommendations.map(ex => (
                    <ApexCard key={ex.id} className="p-6 space-y-4">
                      <div className="flex justify-between items-start gap-4">
                        ...
                        <ApexButton variant="outline" size="icon" onClick={() => setAskAIExercise(ex)}><span className="text-[10px] font-black uppercase tracking-widest">askAI</span></ApexButton>
                      </div>
              ...
              <ApexCard className="p-6 bg-white/5 border-white/10 min-w-[250px]">
                <div className="text-[10px] uppercase tracking-widest text-teal-500 mb-2">Nutrition bridge</div>
                <div className="text-2xl font-black italic mb-3">Fuel Hub</div>
                <div className="text-slate-300 text-sm leading-relaxed mb-5">מעבר מהיר למסך התזונה, ארוחות סביב אימון, hydration ו-supplements.</div>
                <ApexButton variant="premium" className="w-full" onClick={() => setScreen("nutrition")}>לעבור לתזונה</ApexButton>
              </ApexCard>
            </div>
                ...
                return (
                  <ApexCard key={ex.id} className="group flex flex-col h-full hover:border-teal-500/30">
                    <div className="h-64 relative overflow-hidden bg-black/40">
                      <img src={getExerciseImageUrl(ex)} className="w-full h-full object-cover opacity-50 group-hover:scale-110 transition-all duration-1000" onError={(e) => { e.currentTarget.src = getFallbackImage(ex.muscleGroup); }} />
                      <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent" />
                      ...
                      <div className="absolute bottom-6 right-6 left-6 flex justify-between gap-3">
                        <ApexButton variant="outline" size="icon" className="h-14 w-14 bg-black/50 backdrop-blur-xl border-white/10" onClick={() => window.open(getExerciseVideoUrl(ex), "_blank")}><Youtube className="text-rose-500" size={24} /></ApexButton>
                        <div className="flex gap-3">
                          <ApexButton variant="outline" size="icon" className="h-14 w-14 bg-black/50 backdrop-blur-xl border-white/10" onClick={() => setAskAIExercise(ex)}><span className="text-[10px] font-black uppercase tracking-widest">askAI</span></ApexButton>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-12">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <ApexCard className="p-10 space-y-8">
                <div className="flex justify-between items-center">
                  <h3 className="text-2xl font-black italic">הצעות התקדמות</h3>
                  <Sparkles className="text-amber-400" />
                </div>
                <div className="space-y-4">
                  {MASTER_VAULT.slice(0, 5).map(ex => (
              ...
              {history.length > 0 ? history.map(h => (
                <ApexCard key={h.id} className="p-8 flex justify-between items-center bg-slate-900/20 border-white/5 hover:border-teal-500/20 transition-all">
                  <div className="space-y-1">
                    <h5 className="font-black italic uppercase text-xl text-white">Betesh Training Session</h5>
        ...
        {tab === "settings" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
            <ApexCard className="p-10">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-3xl font-black italic">System Control</h3>
                <Settings2 className="text-teal-400" />
              </div>
              <div className="grid md:grid-cols-2 gap-6">
                <ApexCard className="p-6">
                  <div className="text-[10px] uppercase tracking-widest text-slate-500 mb-2">Favorites</div>
                  <div className="text-3xl font-black italic">{favorites.length}</div>
                </ApexCard>
                <ApexCard className="p-6">
                  <div className="text-[10px] uppercase tracking-widest text-slate-500 mb-2">Logged sets</div>
                  <div className="text-3xl font-black italic">{logs.length}</div>
                </ApexCard>
                <ApexCard className="p-6 md:col-span-2">
                  <div className="text-[10px] uppercase tracking-widest text-slate-500 mb-4">Upgrade ideas already added</div>
                  <div className="grid md:grid-cols-2 gap-3">
                    {APP_IDEAS.map(item => <div key={item} className="bg-black/30 rounded-2xl p-4 border border-white/5">{item}</div>)}
                  </div>
                </ApexCard>
              </div>
            </ApexCard>
...
class AppErrorBoundary extends React.Component<{ children: React.ReactNode }, { hasError: boolean; message: string }> {
  ...
  componentDidCatch(error: unknown) {
    console.error("Betesh Training runtime error:", error);
  }

  render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-8" dir="rtl">
          <div className="max-w-2xl w-full bg-slate-900/80 border border-rose-500/30 rounded-[2rem] p-8 space-y-4">
            <div className="text-rose-400 font-black uppercase tracking-widest text-sm">Runtime error</div>
            <h1 className="text-4xl font-black italic">Betesh Training encountered an error</h1>
            <p className="text-slate-300 leading-relaxed">במקום מסך לבן, הנה השגיאה שהאפליקציה קיבלה בזמן ריצה:</p>
            <div className="bg-black/40 border border-white/10 rounded-2xl p-4 font-mono text-sm text-amber-300 break-words">{this.state.message}</div>
            <p className="text-slate-400 text-sm">אחרי שתעלה את הגרסה הזו, אם תהיה תקלה נוספת, תוכל לשלוח לי את הטקסט שמופיע כאן.</p>
          </div>
        </div>
      );
    }
