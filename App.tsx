    ...
    <div className="fixed inset-0 z-[700] bg-black/90 backdrop-blur-2xl flex items-center justify-center p-6" dir="rtl">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-3xl bg-slate-900 border border-white/10 rounded-[3rem] overflow-hidden">
        <div className="relative h-56">
          <img src={exercise.imageUrl} className="w-full h-full object-cover opacity-30" />
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <ApexButton variant="outline" className="flex-1 gap-2" onClick={() => window.open(exercise.videoUrl, "_blank")}><ExternalLink size={18} /> פתח וידאו</ApexButton>
          </div>
        </div>
      </motion.div>
    </div>
...
function ReacherApp() {
  ...
  const playAlert = (f: number) => {
    ...
    g.gain.exponentialRampToValueAtTime(0.01, audioCtx.current.currentTime + 0.35);
    o.start();
    o.stop(audioCtx.current.currentTime + 0.35);
  };
    ...
    return (
      <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-center p-12 overflow-hidden text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 space-y-12">
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.4em] text-sm">Training, nutrition, recovery, progression.</p>
          <div className="flex flex-col md:flex-row gap-4 justify-center">
            <ApexButton size="lg" className="px-20 py-10 text-2xl" onClick={() => setScreen("nutrition")}>ENTER HUB</ApexButton>
          </div>
        </motion.div>
      </div>
    );
      ...
      <header className="relative z-10 max-w-7xl mx-auto px-6 pt-12 pb-8 flex justify-between items-start">
        <div className="space-y-3">
          <div className="flex items-center gap-4">
            <h1 className="text-6xl md:text-7xl font-black italic uppercase tracking-tighter leading-none">REACHER<span className="text-teal-500">APEX</span></h1>
            <ApexButton variant="premium" className="gap-2 rounded-[1.4rem]" onClick={() => setAskAIExercise(dayRecommendations[0] || MASTER_VAULT[0])}><Bot size={18} /> askAI</ApexButton>
          </div>
                ...
                {dayRecommendations.map(ex => (
                  <ApexCard key={ex.id} className="p-6 space-y-4">
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <ApexButton variant="outline" size="icon" onClick={() => setAskAIExercise(ex)}><Bot size={16} /></ApexButton>
                    </div>
                  ...
                  <ApexCard key={ex.id} className="group flex flex-col h-full hover:border-teal-500/30">
                    <div className="h-64 relative overflow-hidden bg-black/40">
                      ...
                      <div className="absolute bottom-6 right-6 left-6 flex justify-between gap-3">
                        <ApexButton variant="outline" size="icon" className="h-14 w-14 bg-black/50 backdrop-blur-xl border-white/10" onClick={() => window.open(ex.videoUrl, "_blank")}><ExternalLink className="text-rose-500" size={24} /></ApexButton>
                        <div className="flex gap-3">
                          <ApexButton variant="outline" size="icon" className="h-14 w-14 bg-black/50 backdrop-blur-xl border-white/10" onClick={() => setAskAIExercise(ex)}><Bot size={22} /></ApexButton>
