...
const exerciseImageFallback: Record<MuscleGroup, string> = {
  Back: "https://images.unsplash.com/photo-1603287611837-f2146f5de8e8?q=80&w=800",
  Chest: "https://images.unsplash.com/photo-1571019614242-c5c5dee9f50b?q=80&w=800",
  Legs: "https://images.unsplash.com/photo-1574680096145-d05b474e2155?q=80&w=800",
  Shoulders: "https://images.unsplash.com/photo-1591741535018-d042766c62eb?q=80&w=800",
  Arms: "https://images.unsplash.com/photo-1583454110551-21f2fa2afe61?q=80&w=800",
  Core: "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?q=80&w=800",
  FullBody: REACHER_HERO
};

function getSafeImage(exercise: Exercise) {
  return exercise.imageUrl?.trim() ? exercise.imageUrl : exerciseImageFallback[exercise.muscleGroup];
}

function handleExerciseImageError(event: React.SyntheticEvent<HTMLImageElement>, exercise: Exercise) {
  const target = event.currentTarget;
  const fallback = exerciseImageFallback[exercise.muscleGroup];
  if (target.src !== fallback) target.src = fallback;
}

function openExerciseVideo(exercise: Exercise) {
...
const nutritionExtras = [
  { title: "Hydration Planner", body: "35-45 מ״ל מים לכל ק״ג גוף, ועוד 500-750 מ״ל סביב אימון. באימונים חמים, תוסיף גם אלקטרוליטים." },
  { title: "Protein Target", body: "כוון ל-1.6 עד 2.2 גרם חלבון לכל ק״ג ליום. חלק ל-3 עד 5 ארוחות כדי לשמור על סינתזת חלבון גבוהה." },
  { title: "Pre Workout Fuel", body: "60-90 דקות לפני אימון, בחר חלבון קל לעיכול ופחמימה זמינה. לדוגמה: יוגורט, בננה, טוסט, שיבולת שועל." },
  { title: "Recovery Meal", body: "אחרי אימון, תן עדיפות לחלבון מלא, פחמימה וסודיום. זה מחזיר אנרגיה, נוזלים וביצועים לאימון הבא." },
  { title: "Supplement Stack", body: "קריאטין, ויטמין D לפי צורך, מגנזיום בלילה, וקפאין לפי סבילות. לא חובה, אבל יכול לעזור לביצועים ועקביות." },
  { title: "Shopping Builder", body: "תחזיק תמיד בסיס קבוע: ביצים, יוגורט, עוף, טונה, אורז, שיבולת שועל, לחם טוב, פירות, ירקות ואגוזים." }
];

const splashCards = [
  { key: "nutrition" as const, title: "Nutrition Hub", subtitle: "תזונה, הידרציה, התאוששות ומאקרו", cta: "Enter Nutrition" },
  { key: "main" as const, title: "Training Board", subtitle: "אימונים, מאגר תרגילים, AI, לוגים וביצועים", cta: "Enter Training" }
];
    ...
    <div className="fixed inset-0 z-[700] bg-black/90 backdrop-blur-2xl flex items-center justify-center p-6" dir="rtl">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-3xl bg-slate-900 border border-white/10 rounded-[3rem] overflow-hidden">
        <div className="relative h-56">
          <img src={getSafeImage(exercise)} onError={(e) => handleExerciseImageError(e, exercise)} className="w-full h-full object-cover opacity-30" />
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <ApexButton variant="outline" className="flex-1 gap-2" onClick={() => openExerciseVideo(exercise)}><ExternalLink size={18} /> פתח ביוטיוב</ApexButton>
          </div>
        </div>
      </motion.div>
    </div>
          ...
          <div className="grid md:grid-cols-2 gap-4 max-h-[60vh] overflow-y-auto pr-2 scrollbar-hide">
            {alts.map(ex => (
              <ApexCard key={ex.id} className="p-4 flex items-center gap-4 cursor-pointer hover:bg-white/5" onClick={() => { onSwap(ex); onClose(); }}>
                <img src={getSafeImage(ex)} onError={(e) => handleExerciseImageError(e, ex)} className="w-20 h-20 rounded-2xl object-cover opacity-50" />
...
function ReacherApp() {
  ...
  const playAlert = (f: number, duration = 0.35, volume = 0.08) => {
  ...
  const playSound = (type: "enter" | "nutrition" | "add" | "start" | "log" | "done" | "back" | "toggle") => {
    const map = {
      enter: [680, 920],
      nutrition: [520, 700],
      add: [980],
    ...
    map[type].forEach((freq, idx) => {
      window.setTimeout(() => playAlert(freq, 0.22, 0.06), idx * 90);
    });
  };
      ...
      <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-center p-12 overflow-hidden text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 space-y-12">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-teal-400 font-mono text-[10px] uppercase tracking-[0.6em] mt-4"><Cpu size={14} className="inline mr-2" /> BETESH TRAINING SYSTEM v21.0</p>
          </div>
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.4em] text-sm">Created by Noam Betesh</p>
          <div className="flex flex-col md:flex-row gap-4 justify-center">
            <ApexButton size="lg" className="px-20 py-10 text-2xl" onClick={() => setScreen("nutrition")}>CHOOSE MODE</ApexButton>
          </div>
        </motion.div>
      </div>
    );
      ...
      <div className="min-h-screen bg-[#020617] text-white p-6 md:p-12" dir="rtl">
        <div className="max-w-7xl mx-auto space-y-10">
          <header className="flex justify-between items-center">
            ...
            <div className="flex gap-3"><ApexButton variant="outline" className="gap-2" onClick={() => { playSound("back"); setScreen("splash"); }}><Home size={18} /> מסך ראשי</ApexButton><ApexButton variant="premium" className="gap-2" onClick={() => { playSound("enter"); setScreen("main"); }}><ArrowRightLeft size={18} /> לעבור לאימון</ApexButton></div>
          </header>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            {nutritionExtras.map(item => (
              <ApexCard key={item.title} className="p-6 space-y-3 bg-white/5 border-white/10">
                <div className="text-teal-400 font-black uppercase tracking-widest text-[10px]">Nutrition extra</div>
      ...
      <header className="relative z-10 max-w-7xl mx-auto px-6 pt-12 pb-8 flex justify-between items-start">
        <div className="space-y-3">
          <div className="flex items-center gap-4">
            ...
            <ApexButton variant="premium" className="gap-2 rounded-[1.4rem]" onClick={() => setAskAIExercise(dayRecommendations[0] || MASTER_VAULT[0])}>askAI</ApexButton>
          </div>
        ...
        <div className="flex bg-slate-900/80 p-2 rounded-[2.5rem] w-full max-w-2xl mx-auto mb-16 border border-white/10 backdrop-blur-3xl shadow-2xl">
          {[
            ...
            <button
              ...
              style={{ fontFamily: "Trebuchet MS, Arial Black, Arial, sans-serif", letterSpacing: "0.03em" }}
            >
                ...
                {dayRecommendations.map(ex => (
                  <ApexCard key={ex.id} className="p-6 space-y-4">
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <ApexButton variant="outline" onClick={() => setAskAIExercise(ex)}>askAI</ApexButton>
                    </div>
                ...
                return (
                  <ApexCard key={ex.id} className="group flex flex-col h-full hover:border-teal-500/30">
                    <div className="h-64 relative overflow-hidden bg-black/40">
                      <img src={getSafeImage(ex)} onError={(e) => handleExerciseImageError(e, ex)} className="w-full h-full object-cover opacity-50 group-hover:scale-110 transition-all duration-1000" />
                      <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent" />
                      ...
                      <div className="absolute bottom-6 right-6 left-6 flex justify-between gap-3">
                        <ApexButton variant="outline" size="icon" className="h-14 w-14 bg-black/50 backdrop-blur-xl border-white/10" onClick={() => openExerciseVideo(ex)}><ExternalLink className="text-rose-500" size={24} /></ApexButton>
                        <div className="flex gap-3">
                          <ApexButton variant="outline" className="h-14 px-5 bg-black/50 backdrop-blur-xl border-white/10" onClick={() => setAskAIExercise(ex)}>askAI</ApexButton>
