...
const EXERCISES: Exercise[] = [
  ...
  { id: "gym_b8", name: "Pendlay Row", sets: 4, reps: "8", he: "חתירה מתפרצת מהרצפה. כוח מתפרץ וגב עוצמתי.", work: 45, rest: 120, category: "pull", muscleGroup: "Back", videoUrl: "https://www.youtube.com/results?search_query=Pendlay+Row+technique", imageUrl: "https://images.unsplash.com/photo-1517838277536-f5f99be501cd?q=80&w=1200", difficulty: "Elite", sector: "Gym" },
...
function ReacherApp() {
  ...
  const renderNutrition = () => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
      ...
      <div className="grid lg:grid-cols-4 gap-4">
        ...
        <Card className="p-5 bg-orange-500/10 border-orange-400/20"><div className="flex items-center gap-2 mb-3"><UtensilsCrossed className="text-orange-300" /><div className="font-black">ארוחה הבאה</div></div><div className="text-2xl font-black italic text-orange-300">{nextMealLabel}</div></Card>
        <Card className="p-5 bg-violet-500/10 border-violet-400/20"><div className="flex items-center gap-2 mb-3"><Target className="text-violet-300" /><div className="font-black">קלוריות היום</div></div><div className="text-3xl font-black italic text-violet-300">{totalNutritionCalories}</div></Card>
      </div>
      <Card className="p-6">
        ...
        <div className="mt-6"><Btn variant="premium" onClick={() => setQuickAddMenuOpen(true)}><Plus size={16} /> הוספה מהירה</Btn></div>
      </Card>
      <div className="grid lg:grid-cols-2 gap-6">
        <Card className="p-6"><h3 className="text-2xl font-black italic mb-4 text-white">ארוחות היום</h3><div className="space-y-3">{todayMeals.length ? todayMeals.map((entry) => <div key={entry.id} className="bg-black/30 border border-white/5 rounded-2xl p-4 flex gap-3"><div className="flex-1"><div className="font-black italic">{entry.title}</div><div className="text-xs text-slate-400">{entry.calories} קלוריות</div></div><button onClick={() => setNutritionEntries((prev) => prev.filter((item) => item.id !== entry.id))} className="text-rose-400"><Trash2 size={16} /></button></div>) : <div className="text-slate-400">אין ארוחות.</div>}</div></Card>
        <Card className="p-6"><h3 className="text-2xl font-black italic mb-4 text-white">מדידה אחרונה</h3>{latestMeasurement ? <div className="text-slate-200">משקל: {latestMeasurement.weight} ק"ג</div> : <div className="text-slate-400">אין מדידות.</div>}</Card>
      </div>
    </motion.div>
  );
  ...
  const renderCardio = () => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
      <Card className="p-6"><h2 className="text-3xl md:text-4xl font-black italic">מרכז אירובי חכם</h2><div className="text-slate-300 mt-4">מותאם לפי המטרות וה-BMI שלך.</div></Card>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-5"><div className="text-xs text-slate-500 mb-1">Zone 2 שבועי</div><div className="text-3xl font-black italic text-orange-400">{cardioPlan.zone2Minutes} דק׳</div></Card>
        <Card className="p-5"><div className="text-xs text-slate-500 mb-1">HIIT שבועי</div><div className="text-3xl font-black italic text-rose-400">{cardioPlan.hiitMinutes} דק׳</div></Card>
        <Card className="p-5"><div className="text-xs text-slate-500 mb-1">צעדים מומלצים</div><div className="text-3xl font-black italic text-cyan-400">{cardioPlan.walkingSteps}</div></Card>
      </div>
      <Card className="p-6"><div className="font-black italic text-2xl mb-4">המלצת ביצוע</div><div className="text-slate-300 leading-relaxed">{cardioPlan.paceTip}</div></Card>
    </motion.div>
  );
      ...
      <AnimatePresence>
        {showNamePrompt && (
          <div className="fixed inset-0 z-[950] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="w-full max-w-md bg-slate-900 border border-white/10 rounded-[2rem] p-6 text-right">
