...
const dayImageByKey: Record<string, string> = {
  day1: imageByMuscle.Back,
  day2: imageByMuscle.Chest,
  day3: imageByMuscle.Legs,
  day4: imageByMuscle.Shoulders,
  day5: imageByMuscle.FullBody,
};

const heroImage =
  "https://images.unsplash.com/photo-1517836357463-d25dfeac3438?q=80&w=1400&auto=format&fit=crop";

const motivationLine = "Built Different. Train Ruthless.";
...
function openYouTubeApp(url?: string) {
  if (typeof window === "undefined") return;
  const fallback = url || "https://www.youtube.com/";
  const now = Date.now();
...
function getWeekProgressKey(dayKey: string) {
  const d = new Date();
  d.setUTCDate(d.getUTCDate() + 4 - (d.getUTCDay() || 7));
  const weekKey = `${d.getUTCFullYear()}-W${Math.ceil((((d.getTime() - new Date(Date.UTC(d.getUTCFullYear(), 0, 1)).getTime()) / 86400000) + 1) / 7)}`;
...
function ReacherApp() {
  ...
  const [voiceOn, setVoiceOn] = useLocalStorage("reacher_voice", true);
  ...
  const displayExercises = useMemo(() => {
    return selectedMuscle
      ? allExercisesPool.filter((ex) => ex.muscleGroup === selectedMuscle)
      : selectedDay.exercises;
  }, [selectedMuscle, selectedDay]);

  function findExerciseLocation(exerciseId: string) {
    for (const day of days) {
      const idx = day.exercises.findIndex((e) => e.id === exerciseId);
      if (idx !== -1) return { dayKey: day.key, index: idx };
    }
    return null;
  }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        if (soundOn) playBeep();
        if (phase === "rest") {
          if (pushEnabled) notify("Time to lift!", `Rest is over. Get ready for ${liveExercise?.name}`);
          speakCue("Rest is over. Back to work.");
        } else {
  ...
  },[running, targetTime, autoAdvance, soundOn, phase, liveExercise, pushEnabled, notify, speakCue]);
      ...
      setSetIndex((currSetIdx) => {
        ...
        if (phase === "work") {
          ...
          if (currSetIdx + 1 >= ex.sets) {
            if (currExIdx + 1 >= selectedDay.exercises.length) { setPhase("done"); setRunning(false); setTargetTime(null); speakCue("Workout complete. Great job."); setWeeklyProgress(prev => ({ ...prev,[`${getCurrentWeekKey()}-${selectedDay.key}`]: true })); return currSetIdx; }
          }
  ...
  const workoutDaysThisMonth = useMemo(() => {
    ...
    Object.values(exerciseHistory).forEach((records) => {
      records.forEach((r) => {
        ...
        stamps.add(`${d.getFullYear()}-${d.getMonth()}-${d.getDate()}`);
      });
    });
    return stamps;
  }, [exerciseHistory]);
  ...
  const calendarDays = useMemo(() => {
    ...
    return Array.from({ length: totalCells }, (_, i) => {
      ...
      return { dayNumber, hasWorkout: workoutDaysThisMonth.has(key), isToday: dayNumber === now.getDate() };
    });
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-50 font-sans selection:bg-zinc-800 relative pb-10">
      ...
      <div className="mx-auto max-w-7xl p-4 md:p-6 lg:p-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <div className="overflow-hidden rounded-[2.25rem] border border-zinc-800/70 bg-zinc-950/65 shadow-[0_24px_90px_rgba(0,0,0,0.45)] backdrop-blur-sm">
            <div className="relative h-[280px] md:h-[360px]">
              <img src={heroImage} alt="Betesh BIST trainer" className="h-full w-full object-cover opacity-70" />
              <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(17,24,39,0.15),rgba(9,11,18,0.82))]" />
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.06),transparent_42%)]" />
              <div className="absolute inset-x-0 bottom-0 p-6 md:p-8 text-center" dir="rtl">
                <div className="inline-flex items-center rounded-full border border-white/10 bg-black/30 px-3 py-1 text-xs text-zinc-200 backdrop-blur">
                  Betesh BIST trainer
                </div>
                <div className="mx-auto mt-5 max-w-3xl">
                  <h1 className="text-3xl md:text-6xl font-black tracking-[0.08em] uppercase text-white [text-shadow:0_8px_30px_rgba(0,0,0,0.6)]">
                    {motivationLine}
                  </h1>
                </div>
                <div className="mt-6 flex flex-wrap justify-center gap-3">
                  <button
                    ...
                    className="inline-flex items-center rounded-2xl border border-emerald-500/30 bg-emerald-500/15 px-4 py-2 text-sm font-bold text-emerald-300 hover:bg-emerald-500/25"
                  >
                    פתח Spotify
                  </button>
                  <Button
                    variant="outline"
                    className="rounded-2xl bg-black/30 backdrop-blur"
            ...
            <div className="border-t border-zinc-800/70 bg-zinc-950/85 p-5 md:p-6">
              <div className="flex items-center justify-between text-sm font-semibold text-zinc-300" dir="rtl">
                <button
                  type="button"
                  onClick={() => setScreen("analytics")}
                  className="inline-flex items-center gap-2 rounded-xl border border-zinc-800 bg-zinc-900/70 px-3 py-2 hover:bg-zinc-900"
                >
                  <span>לוח שבועי</span>
              ...
              <div className="mt-4 flex flex-wrap gap-2 justify-center md:justify-start" dir="rtl">
                {initialDaysWithMedia.map((d) => {
                  const progressKey = getWeekProgressKey(d.key);
                  const isDone = weeklyProgress[progressKey];
                  return (
                    <button
                      key={d.key}
                      type="button"
                      ...
                      className={`h-11 min-w-[64px] px-3 flex items-center justify-center rounded-2xl border text-sm font-bold transition ${
                        isDone
        ...
        {screen !== "splash" && <div className="grid h-auto w-full max-w-4xl grid-cols-5 gap-1 rounded-2xl border border-zinc-800/60 bg-zinc-900/40 p-1.5 mb-8">
        ...
        {screen === "splash" && (
          <div className="min-h-[70vh] flex items-center justify-center">
            <motion.div
              ...
              className="w-full max-w-4xl overflow-hidden rounded-[2.5rem] border border-zinc-800/70 bg-zinc-950/60 shadow-[0_24px_90px_rgba(0,0,0,0.45)] backdrop-blur-sm"
            >
              <div className="relative h-[420px]">
                <img src={heroImage} alt="Betesh BIST trainer" className="h-full w-full object-cover opacity-70" />
                <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(17,24,39,0.15),rgba(9,11,18,0.88))]" />
                <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
                  <div className="inline-flex items-center rounded-full border border-white/10 bg-black/30 px-3 py-1 text-xs text-zinc-200 backdrop-blur">
                    Betesh BIST trainer
                  </div>
                  <h1 className="mt-6 text-4xl md:text-7xl font-black tracking-[0.1em] uppercase text-white [text-shadow:0_8px_30px_rgba(0,0,0,0.6)]">
                    {motivationLine}
                  </h1>
                  <p className="mt-4 max-w-2xl text-sm md:text-base text-zinc-300" dir="rtl">
                    תכנון אימונים, מצב לייב, AskAI, מעקב התקדמות, פתיחה מהירה לספוטיפיי ויוטיוב, והכל במקום אחד.
                  </p>
                  <div className="mt-8 flex flex-wrap gap-3 justify-center">
                    <Button className="rounded-2xl px-6" onClick={() => setScreen("home")}>כניסה לאפליקציה</Button>
                    <Button variant="outline" className="rounded-2xl" onClick={openSpotifyApp}>פתח Spotify</Button>
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        {screen === "home" && (
          <div className="space-y-6">
            <div className="flex max-w-[320px] rounded-2xl border border-zinc-800/70 bg-zinc-900/50 p-2 shadow-lg backdrop-blur-sm" dir="rtl">
...
<div className="grid gap-4 md:grid-cols-[1.2fr_0.8fr]">
  <Card className="overflow-hidden rounded-[2rem] border border-zinc-800/80 bg-zinc-950/70 shadow-[0_14px_50px_rgba(0,0,0,0.35)] backdrop-blur-sm">
    <CardContent className="p-5 md:p-6 text-right" dir="rtl">
      <div className="text-lg font-bold">כניסה מהירה</div>
      <div className="mt-2 text-sm leading-6 text-zinc-300">
        בחר יום אימון, עבור ללייב, או פתח קבוצת שרירים כדי לראות את כל התרגילים שלה במקום אחד.
      </div>
    </CardContent>
  </Card>
  <button
    type="button"
    onClick={openSpotifyApp}
    className="rounded-[2rem] border border-emerald-500/25 bg-[linear-gradient(135deg,rgba(16,185,129,0.16),rgba(16,185,129,0.04))] p-5 md:p-6 shadow-[0_14px_50px_rgba(0,0,0,0.3)] transition hover:-translate-y-1 text-right"
  >
    <div className="text-right" dir="rtl">
      <div className="text-lg font-bold text-emerald-300">Spotify לאימון</div>
      <div className="mt-2 text-sm leading-6 text-zinc-200">
        פתח מוזיקה ברקע לאימון שלך בלחיצה אחת.
      </div>
    </div>
  </button>
</div>
...
<div className="grid gap-4 md:grid-cols-3">
  <Card className="rounded-[2rem] border border-zinc-800/80 bg-zinc-950/70 shadow-[0_14px_50px_rgba(0,0,0,0.35)] backdrop-blur-sm">
    <CardContent className="p-5 text-right" dir="rtl">
      <div className="text-base font-bold">מעבר מהיר ללייב</div>
      <div className="mt-2 text-sm leading-6 text-zinc-300">
        בחר יום והתחל את האימון במסך אחד, בלי ללכת לאיבוד בתפריטים.
      </div>
    </CardContent>
  </Card>
  <Card className="rounded-[2rem] border border-zinc-800/80 bg-zinc-950/70 shadow-[0_14px_50px_rgba(0,0,0,0.35)] backdrop-blur-sm">
    <CardContent className="p-5 text-right" dir="rtl">
      <div className="text-base font-bold">AskAI לכל תרגיל</div>
      <div className="mt-2 text-sm leading-6 text-zinc-300">
        בלחיצה אחת אפשר לקבל דגשים, טכניקה ורמזים לביצוע נכון.
      </div>
    </CardContent>
  </Card>
  <Card className="rounded-[2rem] border border-zinc-800/80 bg-zinc-950/70 shadow-[0_14px_50px_rgba(0,0,0,0.35)] backdrop-blur-sm">
    <CardContent className="p-5 text-right" dir="rtl">
      <div className="text-base font-bold">וידאו ישירות לאפליקציה</div>
      <div className="mt-2 text-sm leading-6 text-zinc-300">
        כפתור הסרטון מנסה לפתוח את YouTube באפליקציה קודם, ואז עובר לדפדפן רק אם צריך.
      </div>
    </CardContent>
  </Card>
</div>
            ...
            <AnimatePresence mode="wait">
              {viewMode === "days" ? (
                <motion.div key="days" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" dir="rtl">
                  {days.map((day) => (
                    <Card key={day.key} className="overflow-hidden rounded-[2rem] border border-zinc-800/80 bg-zinc-950/80 shadow-[0_14px_50px_rgba(0,0,0,0.35)] backdrop-blur-sm transition hover:-translate-y-1 hover:border-zinc-700">
                      <div className="relative h-56 overflow-hidden">
                        <img
                          src={dayImageByKey[day.key] || day.exercises[0]?.imageUrl || imageByMuscle[day.exercises[0].muscleGroup]}
                          alt={day.title}
                          className="h-full w-full object-cover opacity-65"
                        />
                        <div className={`absolute inset-0 bg-gradient-to-t ${day.accent} from-5% via-transparent to-transparent`} />
                      ...
                      <CardContent className="p-5 text-right">
                        <div className="mb-5 text-sm leading-6 text-zinc-300">{day.focusHe}</div>
                        <div className="grid grid-cols-2 gap-3">
                          <Button variant="outline" className="rounded-2xl" onClick={() => startLive(day.key)}>התחל</Button>
                          <Button className="rounded-2xl" onClick={() => { setSelectedMuscle(null); setSelectedDayKey(day.key); setScreen("day"); }}>פרטים</Button>
                        </div>
                      </CardContent>
                    </Card>
                  ))}
                ...
                <motion.div key="muscles" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" dir="rtl">
                  {Object.keys(imageByMuscle).map((muscle) => {
                    ...
                    return (
                      ...
                      >
                        <Card className="overflow-hidden rounded-[2rem] border border-zinc-800/80 bg-zinc-950/80 shadow-[0_14px_50px_rgba(0,0,0,0.35)] backdrop-blur-sm transition hover:-translate-y-1 hover:border-zinc-700">
        ...
        {screen === "day" && (
          <div className="space-y-6">
            <ScrollArea className="w-full pb-4">
              <div className="flex gap-3 min-w-max" dir="rtl">
                <Button
                ...
                {days.map((day) => (
                  <Button
                    key={day.key}
                    variant={selectedMuscle === null && selectedDayKey === day.key ? "default" : "outline"}
                    onClick={() => {
                      setSelectedMuscle(null);
              ...
              <div className="space-y-6">
                <div className="flex justify-between" dir="rtl">
                  <h3 className="text-2xl font-bold">
                    {selectedMuscle ? `תרגילי ${muscleHebrew[selectedMuscle]}` : "תוכנית האימון"}
                  </h3>
                  {!selectedMuscle && (
                    <Button onClick={() => startLive(selectedDay.key)}>
                      <Play className="ml-2 h-4 w-4" /> התחל
                    </Button>
                  )}
                </div>
                {displayExercises.map((ex, idx) => {
                  const maxW = Math.max(...(exerciseHistory[ex.id] || []).map(h => h.weight), 0);
                  const location = findExerciseLocation(ex.id);
                  return (
                    <Card key={ex.id}>
                      <CardContent className="p-0">
                        <div className="grid sm:grid-cols-[180px_1fr]">
                          <div className="h-40 sm:h-full relative border-r border-zinc-800">
                            <img src={ex.imageUrl || imageByMuscle[ex.muscleGroup]} className="w-full h-full object-cover opacity-70" />
                            <div className="absolute top-3 left-3 bg-black/60 px-2 py-1 rounded text-xs font-bold">
                          ...
                          <div className="p-5 flex flex-col justify-between">
                            <div>
                              ...
                              <p dir="rtl" className="text-sm text-zinc-400 mb-4">{ex.he}</p>
                            </div>
                            <div className="flex flex-wrap gap-3 items-center">
                              <Badge variant="secondary">{ex.sets} × {ex.reps}</Badge>
                              {maxW > 0 && <Badge className="text-emerald-300">שיא: {maxW}kg</Badge>}
                              ...
                              <Button variant="outline" size="sm" className="rounded-xl" onClick={() => setAiExerciseToAsk(ex)}>AskAI</Button>
                              {ex.videoUrl && (
                                <button
                                  type="button"
                                  onClick={() => openYouTubeApp(ex.videoUrl)}
                              ...
                              {location && (
                                <Button variant="secondary" size="sm" onClick={() => startLive(location.dayKey, location.index)}>
                                  לייב
                                </Button>
                              )}
                            </div>
              ...
              <div className="space-y-6">
                {!selectedMuscle && (
                  <Card>
                    ...
                    <CardHeader><CardTitle>תרגילי בונוס</CardTitle></CardHeader>
                    <CardContent className="space-y-3">
                      {selectedDay.bonus.map((bonus) => (
                        <div key={bonus.id} className="p-4 border border-zinc-800 rounded-2xl">
                          ...
                          <p dir="rtl" className="text-sm text-zinc-400 mt-1">{bonus.he}</p>
                        </div>
                      ))}
                    </CardContent>
                  </Card>
             ...
             <Card>
               <CardHeader>
                 <CardTitle className="flex items-center gap-2">
                   <TrendingUp className="text-emerald-400" /> התקדמות
                 </CardTitle>
               </CardHeader>
                 ...
                 {analyticsData.length > 0 ? analyticsData.map((data, i) => (
                   <div key={i} className="space-y-2">
                     <div className="flex justify-between items-end">
                       ...
                       <span className="text-emerald-400 font-bold">+{data.progressPercent.toFixed(1)}%</span>
                     </div>
                     <div className="h-4 bg-zinc-950 rounded-full relative overflow-hidden">
                       <div className="absolute h-full bg-zinc-700" style={{ width: '40%' }}>
                         <span className="absolute right-2 text-[10px]">{data.firstW}kg</span>
                       </div>
                       {data.progressPercent > 0 && (
                         <div className="absolute left-[40%] h-full bg-emerald-500" style={{ width: `${Math.min(data.progressPercent, 60)}%` }}>
                           <span className="absolute right-2 text-[10px] text-emerald-950 font-bold">{data.lastW}kg</span>
                         </div>
                       )}
                     </div>
                   </div>
                 )) : <div className="text-zinc-500 text-center py-10">שמור אימונים כדי לראות התקדמות!</div>}
               </CardContent>
             </Card>
             <div className="space-y-6">
               <Card>
                 <CardHeader><CardTitle>לוח אימונים חודשי</CardTitle></CardHeader>
                 <CardContent className="space-y-4" dir="rtl">
                   <div className="grid grid-cols-7 gap-2 text-center text-xs text-zinc-500">
                     {["ב", "ג", "ד", "ה", "ו", "ש", "א"].map((d) => <div key={d}>{d}</div>)}
                   </div>
                   <div className="grid grid-cols-7 gap-2">
                     {calendarDays.map((day, idx) => (
                       <div
                         key={idx}
                         className={`aspect-square rounded-2xl border flex items-center justify-center text-sm font-bold ${
                           !day
                             ? "border-transparent"
                             : day.hasWorkout
               ...
               <Card>
                 <CardHeader><CardTitle>מפת נפח</CardTitle></CardHeader>
                 <CardContent>
                   <div className="flex flex-wrap gap-2">
                     {Object.keys(weeklyProgress).reverse().slice(0, 14).map(key => (
                       <div key={key} className="p-3 bg-emerald-500/20 text-emerald-300 rounded-xl text-xs font-bold">{key}</div>
                     ))}
                   </div>
                 </CardContent>
               </Card>
