...
import {
  ...
  CheckCircle2, TimerReset, Volume2, VolumeX, Home, ListChecks,
...
const getExternalLinkSearchUrl = (exerciseName: string) => `https://www.youtube.com/results?search_query=${encodeURIComponent(exerciseName + " exercise tutorial")}`;

function getExerciseVideoUrl(exercise: Exercise) {
  return getExternalLinkSearchUrl(exercise.name);
}
...
function playUiToneSequence(ctx: AudioContext, sequence: number[], stepMs = 110, gainValue = 0.05) {
  try {
    ...
    // Ignore audio playback failures to avoid crashing the UI.
  }
}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-3xl bg-slate-900 border border-white/10 rounded-[3rem] overflow-hidden">
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <ApexButton variant="outline" className="flex-1 gap-2" onClick={() => window.open(getExerciseVideoUrl(exercise), "_blank")}><ExternalLink size={18} /> פתח וידאו</ApexButton>
          </div>
        </div>
      </motion.div>
    </div>
...
function ReacherApp() {
  ...
  const playAlert = (f: number) => {
    const ctx = audioCtx.current || getSafeAudioContext();
    if (!ctx) return;
    ...
    try {
      ...
      playUiToneSequence(audioCtx.current, [f], 120, 0.08);
    } catch {
  ...
  const playSound = (kind: "enter" | "add" | "start" | "save" | "finish" | "swap") => {
    ...
    try {
      ...
      playUiToneSequence(audioCtx.current, bank[kind], 100, kind === "finish" ? 0.06 : 0.05);
    } catch {
      // Ignore audio failures.
    }
  };
                  ...
                  <ApexCard key={ex.id} className="group flex flex-col h-full hover:border-teal-500/30">
                    <div className="h-64 relative overflow-hidden bg-black/40">
                      ...
                      <div className="absolute bottom-6 right-6 left-6 flex justify-between gap-3">
                        <ApexButton variant="outline" size="icon" className="h-14 w-14 bg-black/50 backdrop-blur-xl border-white/10" onClick={() => window.open(getExerciseVideoUrl(ex), "_blank")}><ExternalLink className="text-rose-500" size={24} /></ApexButton>
