    ...
    <div className="grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
      <div className="space-y-6">
        <div className="flex justify-between" dir="rtl">
          <h3 className="text-2xl font-bold">תוכנית האימון</h3>
        ...
        {selectedDay.exercises.map((ex, idx) => {
          const maxW = Math.max(
            ...(exerciseHistory[ex.id] || []).map((h) => h.weight),
            0
          );
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {selectedDay.bonus.map((bonus) => (
              <div
                key={bonus.id}
                className="p-4 border border-zinc-800 rounded-2xl"
              >
...
{screen === "live" && (
  <div className="space-y-6">
    {phase === "done" ? (
      ...
      >
        <CheckCircle2 className="h-32 w-32 text-emerald-400 mb-8" />
        <h2 className="text-6xl font-bold mb-4">אלוף.</h2>
        <Button size="lg" onClick={() => setScreen("home")}>
          סיום
        </Button>
      </motion.div>
    ) : (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="grid gap-6 lg:grid-cols-[1fr_450px]"
      >
        <div className="space-y-6">
          <Card className="relative">
            <div className="absolute top-0 left-0 w-full h-1">
              <div
                className={`h-full bg-gradient-to-r ${selectedDay.accent}`}
                style={{
                  width: `${((setIndex + exerciseIndex * 10) / (selectedDay.exercises.length * 10)) * 100}%`,
                }}
              />
            </div>
            ...
            <CardContent className="p-8 md:p-12">
              <div className="flex justify-between mb-6">
                <Badge variant="outline">{selectedDay.title}</Badge>
                <Badge
                  className={
                    phase === "work"
              ...
              <div className="text-center mb-8">
                ...
                <p dir="rtl" className="text-zinc-400 mb-4">
                  {liveExercise?.he}
                </p>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setAiExerciseToAsk(liveExercise)}
                >
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-zinc-950/50 rounded-3xl p-6 text-center">
                  <div className="text-zinc-500 font-bold mb-2">סט</div>
                  <div className="text-5xl font-black">
                    {setIndex + 1}/{liveExercise?.sets}
                  </div>
                </div>
                <div className="bg-zinc-950/50 rounded-3xl p-6 text-center">
                  <div className="text-zinc-500 font-bold mb-2">טיימר</div>
                  <div className="text-5xl font-black">
              ...
              <div className="bg-zinc-950/80 rounded-3xl p-6 border border-zinc-800">
                <div className="flex justify-between mb-4">
                  ...
                  {previousSetRecord && (
                    <span className="text-emerald-400 text-sm">
                      קודם: {previousSetRecord.weight}kg × {previousSetRecord.reps}
                    </span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-6">
                  <div>
                    <label className="text-xs text-zinc-500 mb-1">משקל</label>
                    <Input
                      type="number"
                      value={currentWeight}
                  ...
                  <div>
                    <label className="text-xs text-zinc-500 mb-1">חזרות</label>
                    <Input
                      type="number"
                      value={currentReps}
                      onChange={(e: any) => setCurrentReps(e.target.value)}
                    />
                  </div>
                  <div>
                    ...
                    <Slider
                      ...
                      max={10}
                      step={0.5}
                      onValueChange={(v: any) => setCurrentRpe(v[0])}
                      className="mt-3"
                    />
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  <Button size="lg" className="flex-1" onClick={handleNextStep}>
                  ...
                  <Button
                    size="lg"
                    variant="outline"
                    className={isWarmup ? "text-amber-400" : ""}
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between items-center bg-zinc-950 p-3 rounded-xl">
                <div className="font-bold text-sm">מעבר אוטומטי</div>
                <Switch checked={autoAdvance} onCheckedChange={setAutoAdvance} />
              </div>

              <Button variant="outline" className="w-full" onClick={previousStep}>
                חזור אחורה
              </Button>
            </CardContent>
          </Card>
            ...
            <CardContent className="overflow-y-auto flex-1 space-y-2 pb-4">
              {exerciseHistory[liveExercise?.id] ? (
                [...exerciseHistory[liveExercise.id]].reverse().map((record, i) => (
                  <div
                    key={i}
                    className="flex justify-between p-3 rounded-lg bg-zinc-950/50 border border-zinc-800"
                  >
                    ...
                    <div>
                      <Badge variant="outline">RPE {record.rpe}</Badge>
                    </div>
                  </div>
                ))
              ) : (
                <div className="text-zinc-500 text-center py-4">
                  עדיין אין סטים שמורים.
                </div>
              )}
            </CardContent>
          </Card>
...
{screen === "analytics" && (
  <div className="grid gap-6 md:grid-cols-2">
        ...
        {analyticsData.length > 0 ? (
          analyticsData.map((data, i) => (
            <div key={i} className="space-y-2">
              ...
              <div className="h-4 bg-zinc-950 rounded-full relative">
                ...
                {data.progressPercent > 0 && (
                  <div
                    className="absolute left-[40%] h-full bg-emerald-500"
                    style={{ width: `${Math.min(data.progressPercent, 60)}%` }}
                  >
                    <span className="absolute right-2 text-[10px] text-emerald-950 font-bold">
                      {data.lastW}kg
                    </span>
                  </div>
                )}
          ...
          <div className="text-zinc-500 text-center py-10">
            שמור אימונים כדי לראות התקדמות!
          </div>
        )}
      </CardContent>
    </Card>
    ...
    <Card>
      <CardHeader>
        <CardTitle>מפת נפח</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex flex-wrap gap-2">
          {Object.keys(weeklyProgress)
            .reverse()
            .slice(0, 14)
            .map((key) => (
              <div
                key={key}
                className="p-3 bg-emerald-500/20 text-emerald-300 rounded-xl text-xs font-bold"
              >
                {key}
              </div>
            ))}
...
{screen === "settings" && (
  <div className="max-w-3xl mx-auto space-y-6">
    <Card className="border-indigo-500/30">
      ...
      <CardContent className="space-y-6" dir="rtl">
        <div className="space-y-3">
          <label className="font-bold text-zinc-200">מפתח Gemini API</label>
          <Input
            type="password"
            value={geminiApiKey}
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-2xl">הגדרות כלליות</CardTitle>
      </CardHeader>
      <CardContent className="space-y-8">
        <div className="flex justify-between items-center text-lg font-bold">
          ...
          <Button
            ...
            className={pushEnabled ? "text-emerald-400" : ""}
          >
            {pushEnabled ? "פעיל" : "הפעל"}
          </Button>
        </div>

        <div className="space-y-4">
          <div className="flex justify-between font-bold">
            <span>בסיס זמן עבודה</span>
            <span className="text-zinc-400">{globalWorkAdjust}%</span>
          </div>
          <Slider
            value={[globalWorkAdjust]}
            min={50}
            max={200}
            step={10}
            onValueChange={(v: any) => setGlobalWorkAdjust(v[0])}
          />
        </div>

        <div className="space-y-4">
          <div className="flex justify-between font-bold">
            <span>בסיס זמן מנוחה</span>
            <span className="text-zinc-400">{globalRestAdjust}%</span>
          </div>
          <Slider
            value={[globalRestAdjust]}
            min={50}
            max={200}
            step={10}
            onValueChange={(v: any) => setGlobalRestAdjust(v[0])}
          />
        </div>
