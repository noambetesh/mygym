...
const initialDays: DayPlan[] =[
  ...
  { key: "day2", title: "Day 2 - Push", subtitle: "Chest & Triceps", focusHe: "מיקוד: חזה עליון, כוח בלחיצות ומסת טריספס", accent: "from-blue-600 via-blue-900 to-zinc-950", accentSoft: "bg-blue-500/15 text-blue-200 border-blue-500/30", exercises:[ { id: "e7", name: "Low-Incline DB Press", sets: 4, reps: "8-10", he: "שיפוע עדין (15-30 מעלות), ירידה עמוקה לחזה עליון.", work: 40, rest: 90, category: "push", muscleGroup: "Chest" }, { id: "e8", name: "Converging Chest Press Machine", sets: 3, reps: "10-12", he: "טווח מלא, דגש על קירוב הידיים בסוף התנועה.", work: 40, rest: 75, category: "push", muscleGroup: "Chest" }, { id: "e9", name: "Cable Crossover (Mid-to-Low)", sets: 3, reps: "12-15", he: "מתיחה מבוקרת וכיווץ חזק לחזה תחתון/אמצעי.", work: 35, rest: 65, category: "push", muscleGroup: "Chest" }, { id: "e10", name: "JM Press", sets: 3, reps: "8-10", he: "שילוב לחיצה צרה ופשיטת מרפקים לכוח בטריספס.", work: 40, rest: 80, category: "push", muscleGroup: "Arms" }, { id: "e11", name: "Katana / Overhead Cable Extension", sets: 3, reps: "12-15", he: "מעולה לראש הארוך של הטריספס בפוזיציה מתוחה.", work: 35, rest: 60, category: "push", muscleGroup: "Arms" }, { id: "e12", name: "Weighted Dips", sets: 3, reps: "8-10", he: "הטיה קלה קדימה לחזה או גוף זקוף לטריספס.", work: 40, rest: 90, category: "push", muscleGroup: "Chest" } ], bonus:[ { id: "b3", name: "Pec Deck Fly", he: "כיווץ מבודד לחזה.", category: "bonus", muscleGroup: "Chest" }, { id: "b4", name: "Rope Pushdown", he: "פינישר לטריספס.", category: "bonus", muscleGroup: "Arms" } ] },
  ...
  { key: "day4", title: "Day 4 - Armor", subtitle: "Shoulders, Upper Back & Neck", focusHe: "מיקוד: כתפיים רחבות (3D), טרפזים ויציבה", accent: "from-violet-600 via-violet-900 to-zinc-950", accentSoft: "bg-violet-500/15 text-violet-200 border-violet-500/30", exercises:[ { id: "e19", name: "Z-Press / Seated DB Press", sets: 4, reps: "8-10", he: "ישיבה על הרצפה נטולת תמיכה - כוח כתפיים וליבה נקי.", work: 40, rest: 90, category: "armor", muscleGroup: "Shoulders" }, { id: "e20", name: "Cable Lateral Raises", sets: 4, reps: "12-15", he: "הכבל שומר על מתח רציף לאורך כל התנועה (לכתף אמצעית).", work: 40, rest: 60, category: "armor", muscleGroup: "Shoulders" }, { id: "e21", name: "Lu Raises", sets: 3, reps: "12", he: "הרמה צידית מלאה עד למעלה, פיתוח כתף וטרפז עליון.", work: 35, rest: 65, category: "armor", muscleGroup: "Shoulders" }, { id: "e22", name: "Chest-Supported Rear Delt Row", sets: 3, reps: "12-15", he: "משיכה גבוהה עם מרפקים החוצה לכתף אחורית.", work: 35, rest: 60, category: "armor", muscleGroup: "Shoulders" }, { id: "e23", name: "Heavy DB Shrugs", sets: 4, reps: "10-12", he: "החזקה של שנייה בשיא הכיווץ.", work: 35, rest: 70, category: "armor", muscleGroup: "Back" }, { id: "e24", name: "Neck Extensions", sets: 3, reps: "15-20", he: "חיזוק צוואר לבניית מראה יציב ופציעות פחותות.", work: 35, rest: 55, category: "armor", muscleGroup: "Back" } ], bonus:[ { id: "b7", name: "Face Pulls", he: "מרפקים גבוהים, בריאות כתף.", category: "bonus", muscleGroup: "Shoulders" }, { id: "b8", name: "Upright Row", he: "כתף אמצעית וטרפזים.", category: "bonus", muscleGroup: "Shoulders" } ] },
...
function AiCoachModal({ exercise, apiKey, onClose }: { exercise: Exercise, apiKey: string, onClose: () => void }) {
  ...
  const quickPrompts =[
    ...
    { label: "גרסת משקולות חופשיות", icon: RefreshCcw, prompt: `אני רוצה להחליף את זה למשקולות חופשיות. מה התחליף המדויק הכי טוב לזה?` }
  ];

  const handleFallback = () => {
    navigator.clipboard.writeText(`אני מתאמן על ${exercise.name} ומכוון למבנה גוף מאסיבי ופונקציונלי (בסגנון ריצ'ר). תן לי דגשים לטכניקה נכונה וטיפ מקצועני אחד בעברית.`).then(() => { window.open("https://gemini.google.com/app", "_blank"); });
  };
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4" dir="rtl">
          {!apiKey ? (
            ...
            <>
              ...
              {loading && <div className="flex justify-end"><div className="bg-zinc-800 border border-zinc-700 text-zinc-400 p-3 rounded-2xl rounded-bl-sm text-sm">...</div></div>}
            </>
          )}
        </div>
        {apiKey && (
          <div className="p-4 border-t border-zinc-800 bg-zinc-950 flex flex-col gap-3" dir="rtl">
             ...
             <div className="flex gap-2">
                ...
                <Input value={question} onChange={(e: any) => setQuestion(e.target.value)} onKeyDown={(e: any) => e.key === 'Enter' && askGemini(question)} placeholder="הקלד שאלה חופשית..." className="flex-1 text-right" />
             </div>
          </div>
        )}
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex flex-col md:flex-row justify-between gap-6 bg-zinc-900/40 p-6 md:p-8 rounded-[2rem] border border-zinc-800/60 shadow-2xl">
            <div>
              ...
              <p dir="rtl" className="mt-3 text-zinc-400">תכנית היפרטרופיה מתקדמת. עוקב אחרי משקלים, RPE ומאמן AI בזמן אמת.</p>
            </div>
